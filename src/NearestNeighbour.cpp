                             ...
                             const vector< vector<float> > testing_set)
{
  ...
  for (int i = 0; i < total_cases; ++i)
  {
    ...
    int target = testing_set[i][size];
#if 0
    for (int j = 0; j < num_attributes_; ++j)
    {
      if (testing_set[i][size + i] == 1)
      {
        target = i + 1;
        break;
      }
    ...
    const vector< vector<float> > training_set)
{
  ...
  for (int i = 0; i < size; ++i)
  {
    ...
    // Get classification of current labeled example.
    for (int j = 0; j < num_attributes_; ++j) // TODO: function for getting the classification or make it the last item in vector.
    {
      if (training_set[i][num_attributes_ + i] == 1)
      {
        n.classification = i + 1;
        break;
      }
    }
  ...
  sort(neighbours.begin(), neighbours.end(), compareDist);
 ...
 */
template<class T> inline T NearestNeighbour::sqr(const T &x)
{
  return x * x;
}
 ...
 */
bool NearestNeighbour::compareDist(const neighbour &a, const neighbour &b)
{
